...
require 'vendor/google/recaptchalib.php';
...
class AuthController extends AbstractActionController {
	...
	function registerAction() {		
		...
		if ($request->isPost ()) {		
			$g_recaptcha_response=$request->getPost('g-recaptcha-response');
			$secret='6Lfd-P4SAAAAAET-nuOA95CnvbjJ4GhZg_o_WgnJ';
			$remoteIp=new RemoteAddress();			
			$reCaptcha=new \ReCaptcha($secret);
			$resp=$reCaptcha->verifyResponse($remoteIp->getIpAddress(), $g_recaptcha_response);
						
			$form->setValidationGroup ( 'username', 'password', 'confirm_password', 'email');							
			...
			if ($form->isValid ()) {
				if ($resp != null && $resp->success) {
					...
					$this->flashMessenger()->addErrorMessage('Sai phần xác thực người dùng');
					$this->redirect()->toRoute('news/auth/register'); 				
				}
