...
class Module implements ViewHelperProviderInterface{
	public function onBootstrap(MvcEvent $e) {		
		...
		$eventManager->attach ( MvcEvent::EVENT_ROUTE, array ($this,'checkAuthAcl'));
	...
	function checkAuthAcl(MvcEvent $e) {
		...
		}else $title_user='guest';		
		
		// xu ly kiem tra acess list
		...
		foreach ( $acl_role as $role => $resource ) {
			...
			foreach ( $resource as $res ) {
				if (! $acl->hasResource ( $res )) {
					...
					//echo 'resource :'.$res." <br> ";
				}
				$acl->allow ( $role, $res );
				//$x=$acl->isAllowed ( $role, $res )? 'true':'false';
				//echo 'acl :'.$role." - >".$res." = ".$x." <br> ";
			}
		}
		// echo '<br>';
		$action = $e->getRouteMatch ()->getParam ( 'action' );
		$controller2 = explode ( '\\', $e->getRouteMatch ()->getParam ( 'controller' ) )[2];
		$route = strtolower($controller2 . '/' . $action);		 
		 //echo $route. '<br>';
		 //echo $title_user . '<br>';
		 //echo $route . '<br>';
		if ($acl->hasResource($route)) {
			if ($acl->isAllowed ( $title_user,$route )) {
				//echo 'Bạn có quyền truy cập trang này ';
			} else {
				//echo 'Bạn không có quyền truy cập trang này ';
				$response = $e->getResponse ();
				$response->getHeaders ()->addHeaderLine ( 'Location', 'http://localhost/workspace/homestockv2/public/thong-bao.html' );
	...
	function checkAuthAclxxx(MvcEvent $e) {
		$controller = $e->getRouteMatch ()->getParam ( 'controller' );
